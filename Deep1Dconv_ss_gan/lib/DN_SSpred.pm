...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(generate_feature_for_convolution newDN make_features all_testfeatures make_testfeatures print_features
	train_DN test_DN all_dnss_files predict_SS make_dnss_file
...
sub features {
	...
	my ($ssafile, $pssmfile, @probfiles) = @{ $file_ref };
	my ($feats_ref, $window, $boost_wind, $reduced) = @{ $opt_ref  };
	my ($pssm, $atch, $seq, $bound, $skipx) = @{ $feats_ref };
...
sub make_dnss_file {
	my ($predfile_ref, $pssmfile, $outfile, $header, $ssafile) = @_;
...
#		}
	...
	# Checks that there is one ss per residue 
	# If not, prints both for observation
	if (@ss != @seq) {
		print_comparison (\@seq, \@ss);
		return 0;
	}
