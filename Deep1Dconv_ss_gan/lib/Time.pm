...
@ISA = qw(Exporter);
@EXPORT = ();
@EXPORT_OK = qw(get_localtime get_time get_date formatted_time format_time formatted_date format_date formatted_localtime difference time_to_secs secs_to_timestopwatch_start stopwatch_stop);
...
sub stopwatch_stop {
	my ($time_ref1) = @_;
	my @times = get_time();
	my $elapsed = difference($time_ref1, \@times);
	return $elapsed;
}
