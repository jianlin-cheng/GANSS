...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=50;$i+=1)
  {
    ...
    print SH "#SBATCH --cpus-per-task=3  # cores per task\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/3_GANSS_CNN_variable_resBased/scripts/train_deepcov_gan_ss_variableLen_residueBased.py 15 $i 5 5 6 50 10 $win  \$feature_dir \$outputdir\n";
