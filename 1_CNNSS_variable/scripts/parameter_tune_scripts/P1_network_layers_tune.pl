...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=20;$i+=1)
  {
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=gpu python /storage/htc/bdm/jh7x3/GANSS/1_CNNSS_variable/scripts/train_deepcov_ss.py 15 5 $i  nadam '6' 100 3 \$feature_dir \$outputdir $win\n";
