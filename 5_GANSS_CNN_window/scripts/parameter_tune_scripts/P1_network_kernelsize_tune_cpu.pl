...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=20;$i+=1)
  {
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/5_GANSS_CNN_window/scripts/train_deepcov_gan_ss.py 10  5 5  $i 50 1000 $win  \$feature_dir \$outputdir\n";
