...
open(OUT,">$outputdir/train_data_filter_summary.txt") || die "Failed to open file $outputdir/train_data_filter_summary.txt\n";
open(OUT2,">$outputdir/validation_filter_data_summary.txt") || die "Failed to open file $outputdir/validation_filter_data_summary.txt\n"; 
...
foreach $dir (sort @subdirs)
{
  ...
  #inter15_filters9_layersGen5_layersDis5_batch10_ftsize6_AA_win1
  $para = substr($dir,index($dir,'filters')+length('filters'),index($dir,'_layersGen')-index($dir,'filters')-length('filters'));
  $win_num = substr($dir,index($dir,'_AA_win')+length('_AA_win'));
  print "Processing filter size $para with winAA $win_num\n";
  ...
  print OUT "$win_num\tKernel\t$para\tQ3\t$q3_train\n";
  print OUT "$win_num\tKernel\t$para\tSOV\t$sov_train\n";
  print OUT "$win_num\tKernel\t$para\tRecon_Err\t$loss_train\n";
  
  
  print OUT2 "$win_num\tKernel\t$para\tQ3\t$q3_val\n";
  print OUT2 "$win_num\tKernel\t$para\tSOV\t$sov_val\n";
  print OUT2 "$win_num\tKernel\t$para\tRecon_Err\t$loss_val\n";
