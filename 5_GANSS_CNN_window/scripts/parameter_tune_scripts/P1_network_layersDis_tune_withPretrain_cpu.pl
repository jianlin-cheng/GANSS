...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=20;$i+=1)
  {
    ...
    print SH "CV_dir=\$outputdir/filters10_layersGen5_layersDis${i}_batch1000_ftsize6_AA_win${win}\n\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python get_postGAN_discriminator_model.py \$CV_dir/model-train-discriminator-deepss_1dconv_gan-best.hdf5 $win 10  ${i}  6   26  3  \$CV_dir/model-train-discriminator-deepss_1dconv_postgan-best.hdf5\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/5_GANSS_CNN_window/scripts/finetune_deepcov_postgan_ss.py 10 5  $i 6 100 1000 $win  \$feature_dir  \$CV_dir/finetune_postgan_model_training \$CV_dir/model-train-discriminator-deepss_1dconv_postgan-best.hdf5\n";
