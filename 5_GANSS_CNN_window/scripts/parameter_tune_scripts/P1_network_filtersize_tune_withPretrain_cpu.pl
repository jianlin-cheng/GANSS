...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=50;$i+=1)
  {
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python get_postGAN_discriminator_model.py \$CV_dir/model-train-discriminator-deepss_1dconv_gan-best.hdf5 $win $i  5  6   26  3  \$CV_dir/model-train-discriminator-deepss_1dconv_postgan-best.hdf5\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/5_GANSS_CNN_window/scripts/finetune_deepcov_postgan_ss.py  $i  5 5 6 100 1000 $win  \$feature_dir  \$CV_dir/finetune_postgan_model_training \$CV_dir/model-train-discriminator-deepss_1dconv_postgan-best.hdf5\n";
