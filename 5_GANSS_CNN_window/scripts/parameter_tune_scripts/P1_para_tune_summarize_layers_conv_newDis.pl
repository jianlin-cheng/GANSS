...
open(OUT,">$outputdir/train_data_layerDis_summary.txt") || die "Failed to open file $outputdir/train_data_layerDis_summary.txt\n";
open(OUT2,">$outputdir/validation_layerDis_data_summary.txt") || die "Failed to open file $outputdir/validation_layerDis_data_summary.txt\n";
...
foreach $dir (sort @subdirs)
{
  ...
  $para = substr($dir,index($dir,'_layersDis')+length('_layersDis'),index($dir,'_batch')-index($dir,'_layersDis')-length('_layersDis'));
  $win_num = substr($dir,index($dir,'_AA_win')+length('_AA_win'));
  print "Processing layer size $para with winAA $win_num\n";
  ...
  print OUT "$win_num\tLayerDis\t$para\tQ3\t$q3_train\n";
  print OUT "$win_num\tLayerDis\t$para\tSOV\t$sov_train\n";
  print OUT "$win_num\tLayerDis\t$para\tRecon_Err\t$loss_train\n";
  
  
  print OUT2 "$win_num\tLayerDis\t$para\tQ3\t$q3_val\n";
  print OUT2 "$win_num\tLayerDis\t$para\tSOV\t$sov_val\n";
  print OUT2 "$win_num\tLayerDis\t$para\tRecon_Err\t$loss_val\n";
