...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=50;$i+=1)
  {
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/5_GANSS_CNN_window/scripts/finetune_deepcov_postgan_ss_withoutPretrain.py  $i  5 5 6 100 1000 $win  \$feature_dir \$CV_dir/finetune_postgan_model_withoutPretraining\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/5_GANSS_CNN_window/scripts/predict_deepcov_ss_postgan.py \$CV_dir/finetune_postgan_model_withoutPretraining/model-train-discriminator-deepss_1dconv_postgan_finetune_nopretrain.hdf5  \$feature_dir  \$CV_dir/finetune_postgan_model_withoutPretraining_evaluation $win\n";
