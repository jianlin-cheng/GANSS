...
if os.path.exists(model_in):
    ...
    discriminator=load_model(model_in)
else:    
...
discriminator_postGAN = build_discriminator_postGAN(AA_win,nb_filters,nb_layers_discriminator,filetsize_array,fea_num,n_class)
...
for ly in range(0,len(discriminator.layers)):
    ...
    if discriminatorname == 'generation':
        continue
