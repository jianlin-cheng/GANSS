...
open(OUT,">$outputdir/train_data_interval_summary.txt") || die "Failed to open file $outputdir/train_data_interval_summary.txt\n";
open(OUT2,">$outputdir/validation_interval_data_summary.txt") || die "Failed to open file $outputdir/validation_interval_data_summary.txt\n";
...
foreach $dir (sort @subdirs)
{
  ...
  $para = substr($dir,index($dir,'filter10_layers10_inter')+length('filter10_layers10_inter'),index($dir,'_optnadam')-index($dir,'filter10_layers10_inter')-length('filter10_layers10_inter'));
  print "Processing Bin size $para\n";
  ...
  print OUT "Bin\t$para\tQ3\t$q3_train\n";
  print OUT "Bin\t$para\tSOV\t$sov_train\n";
  print OUT "Bin\t$para\tLoss\t$loss_train\n";
  
  
  print OUT2 "Bin\t$para\tQ3\t$q3_val\n";
  print OUT2 "Bin\t$para\tSOV\t$sov_val\n";
  print OUT2 "Bin\t$para\tLoss\t$loss_val\n";
