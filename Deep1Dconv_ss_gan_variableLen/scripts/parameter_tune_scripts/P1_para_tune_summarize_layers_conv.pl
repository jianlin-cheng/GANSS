...
open(OUT,">$outputdir/train_data_layer_summary.txt") || die "Failed to open file $outputdir/train_data_layer_summary.txt\n";
open(OUT2,">$outputdir/validation_layer_data_summary.txt") || die "Failed to open file $outputdir/validation_layer_data_summary.txt\n";
...
foreach $dir (sort @subdirs)
{
  ...
  $para = substr($dir,index($dir,'filter10_layers')+length('filter10_layers'),index($dir,'_inter')-index($dir,'filter10_layers')-length('filter10_layers'));
  ...
  print OUT "Layer\t$para\tQ3\t$q3_train\n";
  print OUT "Layer\t$para\tSOV\t$sov_train\n";
  print OUT "Layer\t$para\tLoss\t$loss_train\n";
  
  
  print OUT2 "Layer\t$para\tQ3\t$q3_val\n";
  print OUT2 "Layer\t$para\tSOV\t$sov_val\n";
  print OUT2 "Layer\t$para\tLoss\t$loss_val\n";
