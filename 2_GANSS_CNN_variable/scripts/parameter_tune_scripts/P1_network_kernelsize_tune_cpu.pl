...
for($win=1;$win<=19;$win+=2)
{
  for($i=5;$i<=20;$i+=1)
  {
    ...
    print SH "#!/bin/bash -l\n";
    print SH "#SBATCH -J  ga_kn-$c\n";
    print SH "#SBATCH -o ga_kn-$c-%j.out\n";
    ...
    print SH "THEANO_FLAGS=floatX=float32,device=cpu python /storage/htc/bdm/jh7x3/GANSS/2_GANSS_CNN_variable/scripts/train_deepcov_gan_ss_variableLen.py 15 10 5 5 $i 50 10 $win \$feature_dir \$outputdir\n";
